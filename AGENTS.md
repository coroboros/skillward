...
- `src/bundle.rs` — the digest-pinned bundle image reference
...
- `renovate.json` — the single deps bot (cargo, GitHub Actions, and the bundle image via the `src/bundle.rs` annotation); `pinDigests` preserves the digest pin
